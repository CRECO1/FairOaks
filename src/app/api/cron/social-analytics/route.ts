    ...
    try {
      ...
      if (conn.platform === 'facebook') {
        const res = await fetch(
          `https://graph.facebook.com/v18.0/${conn.page_id}?fields=fan_count,followers_count&access_token=${conn.access_token}`
        );
        const data = await res.json();
        metrics = { followers: data.fan_count || 0 };
      } else if (conn.platform === 'instagram') {
        const res = await fetch(
          `https://graph.facebook.com/v18.0/${conn.platform_account_id}?fields=followers_count,media_count&access_token=${conn.access_token}`
        );
        const data = await res.json();
