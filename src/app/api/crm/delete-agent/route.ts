...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Look up the target's role. Removing an admin or super_admin is a
    // super-admin-only action — this stops an admin (e.g. Brian) from deleting
    // the super admin or another admin to seize control on the way out.
    ...
    if (targetRole === 'admin' || targetRole === 'super_admin') {
      const superCaller = await getCrmSuperAdmin();
      if (!superCaller) return forbidden('Only a super admin can remove an admin.');
    }
