...
import { createSubscription, deleteSubscription, ensureForwardingNumber, getAccount, getPlanInfo, clearHoursOverride, enableCallAnnounce, getHoursOverride, getNumberDestination, setNumberDestination, inspectRouting, isBusinessOpen, listForwardingNumbers, listSubscriptions, botLast, ringThenBot, routeNoAnswerToBot, setAfterHoursViaSchedules, setBusinessHours, setHoursOverride, talkrouteConfigured, TalkrouteError, type TrHourBlock } from '@/lib/talkroute';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (b.action === 'set_after_hours_schedule') {
      // { name, blocks } — humans ring only inside the blocks; otherwise the group falls through to the bot.
      const blocks = (Array.isArray(b.blocks) ? b.blocks : []) as TrHourBlock[];
    ...
    if (b.action === 'bot_last') {
      ...
      const fwd = (await listForwardingNumbers()).find(f => String(f.number ?? '').replace(/\D/g, '').slice(-10) === want);
      if (!fwd?.id) return NextResponse.json({ error: 'Add the bot line to Talkroute forwarding numbers first.' }, { status: 400 });
      return NextResponse.json({ ok: true, ...(await botLast(ringGroupId, seconds, String(fwd.id))) });
    }
