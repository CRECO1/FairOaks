...
export async function POST(req: NextRequest) {
  ...
  if (uploadErr) { console.error('[listing-files] storage upload error:', uploadErr); return NextResponse.json({ error: uploadErr.message }, { status: 500 }); }
  ...
  if (dbErr) { console.error('[listing-files] db insert error:', dbErr); await supabase.storage.from(BUCKET).remove([storagePath]); return NextResponse.json({ error: dbErr.message }, { status: 500 }); }
...
export async function DELETE(req: NextRequest) {
  ...
  const supabase = adminClient();
  const { data: f } = await supabase.from('crm_listing_files').select('storage_path, uploaded_by').eq('id', fileId).single();
