...
export async function GET(req: NextRequest) {
  ...
  // Brand the page per business unit so a commercial contact never sees "Fair Oaks".
  const brand = client.business_unit === 'commercial' ? 'CRECO' : 'Fair Oaks Realty Group';
  return new Response(`<!DOCTYPE html><html><body style="font-family:sans-serif;text-align:center;padding:60px;color:#333"><h2 style="color:#1a1a2e">You've been unsubscribed</h2><p>You will no longer receive campaign messages from <strong>${brand}</strong>.</p><p style="color:#999;font-size:13px;margin-top:24px">To re-subscribe, contact your agent directly.</p></body></html>`, { headers: { 'Content-Type': 'text/html' } });
}
